...
namespace Shriek.Extensions.Dapper
{
    ...
    {
        private readonly Lazy<IDbConnection> internalConnection;
        ...
        {
            DbActions.Enqueue(new DbAction<T>(DbActionTypes.Insert, entity));
        }
        ...
        {
            DbActions.Enqueue(new DbAction<T>(DbActionTypes.Update, entity));
        }
        ...
        {
            DbActions.Enqueue(new DbAction<T>(DbActionTypes.Delete, entity));
        }
        ...
        private DbActionTypes GetActionTypeOfAction(object obj)
        {
            var memberExpr = Expression.PropertyOrField(Expression.Constant(obj), "ActionType");
            return (DbActionTypes)Expression.Lambda(memberExpr).Compile().DynamicInvoke();
        }
