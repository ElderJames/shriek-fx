...
using Shriek.ServiceProxy.DotNetty.ActionAttributes;
...
namespace Shriek.ServiceProxy.DotNetty.Context
{
	...
	internal class AspectContext
	{
		...
		private static AspectContext GetContextNoCache(MethodInfo method)
		{
			...
			var hostAttribute = GetAttributeFromMethodOrInterface<EndPointAttribute>(method, false) ?? new EndPointAttribute(null);
		...
		private static ApiActionDescriptor GetActionDescriptor(MethodInfo method)
		{
			...
			//if (!descriptor.Attributes.Any(x => x is ApiActionAttribute))
